...
function calcularResumen() {
    ...
    const domingosTrab = resumen.domingosTrabajados || 0;
    const esHab = resumen.esHabitual || false;
    let domingoInfo = '';
    if (domingosTrab > 0) {
        const tipo = esHab ? 'HABITUAL' : 'OCASIONAL';
        domingoInfo = `<p style="color: #ffa500;">⚠ Domingos trabajados: ${domingosTrab} (${tipo})</p>`;
        if (esHab) {
            domingoInfo += `<p style="color: #ffa500;">Se deben otorgar ${domingosTrab} días compensatorios.</p>`;
        } else {
            const domingosSinComp = resumen.domingosFechas.filter(f => !resumen.compensatoriosPorDomingo[f]).length;
            if (domingosSinComp > 0) {
                domingoInfo += `<p style="color: #ffa500;">${domingosSinComp} domingo(s) sin compensatorio (recargo 1.8 aplicado).</p>`;
            }
        }
    }
    ...
    cont.innerHTML = `
        ...
        ${domingoInfo}
        <p>Recargo nocturno: ${Math.round(resumen.totalRecargoNocturno || 0).toLocaleString('es-CO', { style:'currency', currency:'COP' })} · Horas nocturnas: ${Math.round(resumen.totalNightHours || 0)}h</p>
        <p>Recargo festivo: ${Math.round(resumen.totalRecargoFestivo || 0).toLocaleString('es-CO', { style:'currency', currency:'COP' })} · Recargo domingo: ${Math.round(resumen.totalRecargoDomingo || 0).toLocaleString('es-CO', { style:'currency', currency:'COP' })}</p>
        <p>Total recargos: ${Math.round(resumen.totalRecargos || 0).toLocaleString('es-CO', { style:'currency', currency:'COP' })}</p>
        <p>Extras manuales: ${Math.round(resumen.totalExtrasManuales || 0)}h · Extras automáticas (>44h/sem): ${Math.round(resumen.totalExtrasAuto || 0)}h</p>
        <p>Total extras (valor): ${Math.round(resumen.totalExtras || 0).toLocaleString('es-CO', { style:'currency', currency:'COP' })} · Total horas: ${Math.round(resumen.totalExtrasCount || 0)}h</p>
        <p>Bruto (salario + recargos + extras): ${Math.round(resumen.brutoMensual || 0).toLocaleString('es-CO', { style:'currency', currency:'COP' })}</p>
        <p>Auxilio transporte: Incluido (a todos los colaboradores)</p>
...
function generarPDF() {
    ...
    const resumen = (typeof window.calculatePayrollSummary === 'function')
        ...
        : { rows: [], totalExtras: 0, totalExtrasCount: 0, totalNightHours: 0, totalRecargoNocturno: 0, totalRecargoFestivo: 0, totalRecargoDomingo: 0, totalRecargos: 0, salud: 0, pension: 0, descuentos: 0, neto: 0, ibc: 0 };

    const doc = new jsPDF();
    doc.setFontSize(14);
    doc.text('Desprendible de Pago - DEspacho_pago', 14, 20);
    doc.setFontSize(10);
    ...
    const recargoDomingoFmt = Math.round(resumen.totalRecargoDomingo || 0).toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0 });
    ...
    const domingosTrab = resumen.domingosTrabajados || 0;
    const esHab = resumen.esHabitual || false;
    const tipoTrabajador = domingosTrab === 0 ? '' : (esHab ? ` (${domingosTrab} domingos - Habitual)` : ` (${domingosTrab} domingos - Ocasional)`);

    doc.text(`Salario base: ${salarioFmt}${tipoTrabajador}`, 14, 44);
    doc.text(`IBC: ${ibcFmt} (incluye aux. transporte)`, 14, 50);
    doc.text(`Recargo nocturno: ${recargoNoctFmt} · Horas nocturnas: ${nightHours}h`, 14, 56);
    doc.text(`Recargo festivo: ${recargoFestFmt} · Recargo domingo: ${recargoDomingoFmt}`, 14, 62);
    const extrasManualesFmt = Math.round(resumen.totalExtrasManuales || 0);
    const extrasAutoFmt = Math.round(resumen.totalExtrasAuto || 0);
    doc.text(`Total recargos: ${recargosFmt}`, 14, 68);
    doc.text(`Extras manuales: ${extrasManualesFmt}h · Extras automáticas (>44h/sem): ${extrasAutoFmt}h · Total: ${extrasFmt} (${extrasCount}h)`, 14, 74);
    doc.text(`Descuentos (Salud ${saludFmt}, Pensión ${pensionFmt}): ${descuentosFmt}`, 14, 80);
    doc.text(`Neto a pagar: ${netoFmt}`, 14, 86);

    const head = [['Fecha', 'Turno', 'Noct(h)', 'R.Noct', 'R.Fest', 'R.Dom', 'Ext.Man', 'Ext.Auto', 'Bruto']];
    const body = resumen.rows.map(r => [
        ...
        Math.round(r.recargoDomingo || 0).toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0 }),
        (r.extrasManuales || 0).toString(),
        (r.extrasAuto || 0).toString(),
        Math.round(r.brutoDia).toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0 })
    ]);

    if (doc.autoTable) {
        doc.autoTable({ startY: 94, head: head, body: body, styles: { fontSize: 7 } });
        
        // Agregar nota sobre días compensatorios si aplica
        let finalY = doc.lastAutoTable.finalY || 94;
        if (resumen.esHabitual && resumen.domingosTrabajados > 0) {
            doc.setFontSize(9);
            doc.text(`⚠ Trabajador HABITUAL: Se deben otorgar ${resumen.domingosTrabajados} días compensatorios.`, 14, finalY + 10);
        } else if (resumen.domingosTrabajados > 0 && resumen.domingosTrabajados <= 2) {
            const domingosSinComp = resumen.domingosFechas.filter(f => !resumen.compensatoriosPorDomingo[f]).length;
            if (domingosSinComp > 0) {
                doc.setFontSize(9);
                doc.text(`⚠ Trabajador OCASIONAL: ${domingosSinComp} domingo(s) sin compensatorio (recargo 1.8 aplicado).`, 14, finalY + 10);
            }
        }
    } else {
        let y = 94;
